# The Wrong Biennale · The Wrong Eclipse · August 2026

| Field | Value |
|-------|--------|
| Organizers | Graziela Calfat, David Quiles Guilló · [The Wrong Biennale](https://thewrong.org/About) |
| Open call | [The Wrong Eclipse](https://thewrong.org/) |
| Submission deadline | **1 August 2026** |
| Exhibition date | **12 August 2026** (24-hour online sound exhibition) |
| Submission email | thewrongbiennale@gmail.com |
| Subject line | `The Wrong Eclipse` |
| Deliverable | Sound work hosted on **Bandcamp** (embedded by curators) |

## Concept (curators)

The eclipse itself is the visual artwork. The exhibition invites listeners to **hear** while experiencing the real eclipse outdoors — less screen time, more sky. Sound only; no video or screen-dependent entries.

## Geographic anchor

The Wrong Biennale — **Partida Maitino 2069c, Alicante, 03295, Spain**. This site is the atmospheric and astronomical clock for works in this folder.

### Eclipse at Maitino / Alicante · 12 August 2026

Partial eclipse only — **no totality**. Maximum obscuration **~99.1%** (~0.9% of the solar disc still visible).

| Event | Time (CEST) |
|-------|-------------|
| First contact | ~19:39:59 |
| Maximum eclipse | ~20:34:59 |
| Above ~99% obscuration | ~20:34 – 20:36 (~2 minutes) |
| Last contact | ~21:25:59 |
| Full partial arc | ~1h 46m |

Sources: [Time.now — Alicante](https://time.now/alicante/eclipses/2026-08-12/) (Maitino is in Alicante province; times match Alicante city within seconds).

## Works in this folder

| Work | File | Status |
|------|------|--------|
| **Holes in the Sky** | [holes-in-the-sky.md](./holes-in-the-sky.md) | In development |

Bandcamp: screenshot of generator UI + binaural L/R audio (PoC). Live re-record **12 August 2026** using atmospheric data above Maitino.

## Path

`catalog/the-wrong-biennale/the-wrong-eclipse-august-2026/`

WIP sketches and code: [`sketches/the-wrong-biennale/the-wrong-eclipse-august-2026/`](../../../sketches/the-wrong-biennale/the-wrong-eclipse-august-2026/).
