/**
 * Canvas + optional audio capture for Loop 21 (R key).
 */
export function createCanvasRecorder(getCanvas, getAudioStream, {
  downloadPrefix = "loop-21-surrender",
} = {}) {
  let recorder = null;
  let chunks = [];
  let active = false;
  ...
  const start = async () => {
    ...
    recorder.onstop = () => {
      ...
      a.href = url;
      a.download = `${downloadPrefix}-${ts}.${recExt(mimeType)}`;
      a.click();
